...
<Callout type="info">
  The fastest path is the one-line install (`uv tool install anamnesis-memory && anamnesis init`), which is live on PyPI today. The PyPI package name is `anamnesis-memory`, but the command it installs is still `anamnesis`. The from-source steps above are for contributors, air-gapped machines, and local development.
</Callout>
