...
<Callout type="info">
Anamnesis is pre-alpha and open source (Apache-2.0). The from-source steps below work today. A shorter one-line install is coming once the package is published to PyPI, and is clearly labeled as not-yet-available further down.
</Callout>
...
## The one-line install (not available yet)

Once Anamnesis is published to the Python package index (PyPI), installing it will be a single command followed by `init`:

```bash
# Not available yet. Coming once the package is published to PyPI.
uv tool install anamnesis-memory && anamnesis init
```

A few honest details so this is not a surprise later:

- This is **not published yet.** Use the from-source steps above for now. The repository README labels this one-liner as something you "will be able to" do once published.
- When it lands, the package on PyPI will be named **`anamnesis-memory`** (the plain name `anamnesis` was already taken by an unrelated project). The command it installs is still just **`anamnesis`**.
- `uv tool install` puts the `anamnesis` command on your PATH globally, so you would no longer need the `uv run` prefix or the cloned `server` folder.
