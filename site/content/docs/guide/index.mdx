...
<Callout type="warn">
Installing Anamnesis today means cloning the repository and building from source. A one-line install from PyPI
(`uv tool install anamnesis-memory && anamnesis init`) is planned, but the package is not published yet, so do
not rely on it until the [Install guide](./install) says it is available.
</Callout>
