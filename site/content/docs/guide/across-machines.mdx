...
<Callout type="info">
The `uv run` prefix is how you run the command from the project checkout today. A shorter one-line
install (`uv tool install anamnesis-memory && anamnesis init`) is planned for once the package is
published to PyPI, but it is not available yet, so this page uses `uv run anamnesis init`
throughout.
</Callout>
